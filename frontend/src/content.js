...
export const ONBOARDING = {
  it: [
    ...
    { target: 'answer', icon: '💬', title: 'La risposta', body: 'Ogni risposta è una “Bozza assistita da AI — da validare”, con un badge di Confidenza. I numeri colorati sono citazioni cliccabili. Gli stati non sono schermate separate: se la confidenza è bassa o la domanda tocca una variabile sensibile, lo vedi qui inline (banner di escalation o warning guardrail).' },
    ...
    { target: 'nav-governance', icon: '📊', title: 'Governance', body: 'La “Coda di revisione” raccoglie le risposte da validare dall’esperto (HITL in pratica). L’“Audit trail” è il registro immutabile (catena SHA-256). Le dashboard separano i “KPI di business” (qualità) dall’“Adoption-health” (adozione nel tempo).' },
    { target: 'input', icon: '⌨️', title: 'La barra di input', body: 'Scrivi qui la tua domanda. Comandi rapidi: /chat (casuale), /chat low (vedi l’escalation) e /chat guardrail (vedi il warning anti-discriminazione). Il pulsante “Domande demo” in basso apre tutto il pool.' },
  ...
  en: [
    ...
    { target: 'answer', icon: '💬', title: 'The answer', body: 'Every answer is an “AI-assisted draft — to be validated”, with a Confidence badge. The coloured numbers are clickable citations. States aren’t separate screens: if confidence is low or the question touches a sensitive variable, you see it inline here (escalation banner or guardrail warning).' },
    ...
    { target: 'nav-governance', icon: '📊', title: 'Governance', body: 'The “Review queue” gathers answers awaiting expert validation (HITL in practice). The “Audit trail” is the immutable log (SHA-256 chain). The dashboards separate “Business KPIs” (quality) from “Adoption health” (adoption over time).' },
    { target: 'input', icon: '⌨️', title: 'The input bar', body: 'Type your question here. Quick commands: /chat (random), /chat low (see escalation) and /chat guardrail (see the anti-discrimination warning). The “Demo questions” button below opens the full pool.' },
